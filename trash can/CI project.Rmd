...
#    print(df1[i])
    t_weighted_mean=mean((df0[i]*df_lo$ipw_wt)[df0$poisox==1,])
    c_weighted_mean=mean((df0[i]*df_lo$ipw_wt)[df0$poisox==0,])
    weighted_mean_diff=abs(t_weighted_mean-c_weighted_mean)
    asam=weighted_mean_diff/sd((df0[i]*df_lo$ipw_wt)[df0$poisox==1,])
...
for(i in colnames(df0)){
  if(i!="poisox" & i!="mortal"){
#    print(df1[i])
    t_weighted_mean=mean((df0[i]*df_rf$ipw_wt)[df0$poisox==1,])
    c_weighted_mean=mean((df0[i]*df_rf$ipw_wt)[df0$poisox==0,])
    weighted_mean_diff=abs(t_weighted_mean-c_weighted_mean)
    asam=weighted_mean_diff/sd((df0[i]*df_rf$ipw_wt)[df0$poisox==1,])
...
for(i in colnames(df0)){
  if(i!="poisox" & i!="mortal"){
#    print(df1[i])
    t_weighted_mean=mean((df0[i]*df1_c$ipw_wt)[df0$poisox==1,])
    c_weighted_mean=mean((df0[i]*df1_c$ipw_wt)[df0$poisox==0,])
    weighted_mean_diff=abs(t_weighted_mean-c_weighted_mean)
    asam=weighted_mean_diff/sd((df0[i]*df1_c$ipw_wt)[df0$poisox==1,])
    cov_balance_c[i]=asam
  }
}
cov_balance_c=cov_balance_c[,-1]
cov_balance_c
cov_balance_c$Average_ASAM=apply(cov_balance_c,1,mean)
cov_balance_c["Average_ASAM"]
```
## Evaluation visualization - Chemical dataset
```{r}
ASAM_table=rbind(cov_balance_lo["Average_ASAM"] ,cov_balance_rf["Average_ASAM"] ,cov_balance_c["Average_ASAM"])
knitr::kable(ASAM_table,"simple")
...
par(mfcol=c(1,3))
boxplot(df_lo[(df0$poisox==0),"ipw_wt"],main="LR")
boxplot(df_rf[(df0$poisox==0),"ipw_wt"],main="RF")
boxplot(df1_c[(df0$poisox==0),"ipw_wt"],main="CART")
mtext("Weight distribution in the Chemical Dataset",outer = TRUE, cex = 2)
...
?par
par(mfcol=c(1,3))
title="Weight distribution in the KHN Dataset"
...
mtext("Weight distribution in the KHN Dataset",outer = TRUE, cex = 2)
```
