...
df$blood_diff=df$subseq-df$prior
df=df[,-3]

colnames(df)[c(3,5)]<-c("treat","outcome")
df %>% head()
dim(df)
...
df_rf=df
rf=randomForest(treat~ .-treat-outcome, data=df_rf,ntree=100, type= "regression")
df_rf$ps=predict(rf)
